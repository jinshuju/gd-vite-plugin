...
export default function vitePluginRequireTransform(
  ...
  return {
    ...
    async transform(code: string, id: string) {
        ...
        traverse(ast, {
          enter(path) {
            if (
              ...
              const name = (
                (path.parentPath.node as t.VariableDeclarator)
                  .id as t.Identifier
              ).name;
            ...
            if (
              ...
              const argument = path.parentPath.node.arguments[0] as
                | t.StringLiteral
                | t.TemplateLiteral;
              ...
              if (isTemplateLiteral) {
                ...
                for (let i = 0; i < tl.quasis.length; i++) {
                  ...
                  const identifier = tl.expressions[i] as
                    | t.Identifier
                    | undefined;
                  const variableValue =
                    (
                      declaredVariables[identifier?.name]?.init as
                        | t.StringLiteral
                        | undefined
                    )?.value ?? "";
              ...
              let originalRequirePath = !isTemplateLiteral
                ? (argument as t.StringLiteral).value
              ...
              if (
                ...
                t.isIdentifier(
                  (path?.parentPath?.parentPath?.node as t.MemberExpression)
                    ?.property
                )
              ) {
                ...
                if (t.isVariableDeclarator(path.parentPath?.parentPath?.node)) {
                  ...
                  variableMather[(variableDeclarator.id as t.Identifier).name] =
                    requirePath;
                }
            ...
            const isRawMethodCheck = (currentExport: string) => {
              return (
                Object.prototype.toString
                  .call(new Array()[currentExport])
                  .includes("Function") ||
                Object.prototype.toString
                  .call(new Object()[currentExport])
                  .includes("Function")
              );
            };
            ...
            if (t.isIdentifier(path.node) && variableMather[path.node?.name]) {
              ...
              const requirePathExports = importMap.get(requirePath);
              const currentExport = (
                (path.parentPath.node as t.MemberExpression)
                  ?.property as t.Identifier
              )?.name;
              if (
                currentExport &&
                !isRawMethodCheck(currentExport) &&
                requirePathExports
              )
                requirePathExports.add(currentExport);
            }
      ...
      variableMather = {};
      return { code: newCode, map: inputMap };
    },
  };
}
